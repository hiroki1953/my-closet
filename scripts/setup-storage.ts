...
async function setupStorage() {
  ...
  try {
    ...
    const profileImagesBucketExists = buckets?.some(
    ...
    const clothingItemsBucketExists = buckets?.some(
      (bucket) => bucket.name === "clothing-items"
    );
    ...
    if (!clothingItemsBucketExists) {
      ...
      const { data: createData, error: createError } =
        await supabaseAdmin.storage.createBucket("clothing-items", {
          public: true,
          allowedMimeTypes: [
            "image/jpeg",
            "image/png",
